...
int AccountList::findAccountByProfileId(const QString& profileId) const
{
	for ( int i = 0; i < count(); i++ )
	{
		...
		if ( account->profileId() == profileId ) { return i; }
	}
	return -1;
}

MinecraftAccountPtr AccountList::getAccountByProfileName(
...
{
	// NOTE: Do not allow adding something that's already there. We shouldn't let
	// it continue because of the signal / slot connections after this.
	...
	if ( profileId.size() )
	{
		...
		{
			qDebug()
			<< "Replacing old account with a new one with the same profile ID!";
			...
			emit dataChanged(index(existingAccount),
...
void AccountList::moveAccount(QModelIndex index, int delta)
{
	...
	if ( index.isValid() && row < m_accounts.size() && newRow >= 0
	     && newRow < m_accounts.size() )
	{
		// Qt is stupid,
		...
		{
			qCritical().noquote()
			<< "AccountList: failed to move account from" << row << "to" << newRow
...
QVariant AccountList::data(const QModelIndex& index, int role) const
{
	...
	switch ( role )
	{
		case Qt::SizeHintRole:
			if ( index.column() == ProfileNameColumn ) { return QSize(0, 30); }
...
QVariant AccountList::headerData(int section,
                          ...
                          int role)
{
	...
	if ( role == Qt::CheckStateRole )
	{
		...
		} else if ( m_defaultAccount == at(idx.row()) ) setDefaultAccount(nullptr);
	}
...
bool AccountList::loadList()
{
	...
	{
		qCritical()
		<< "Can't load Mojang account list. No file path given and no default set.";
		return false;
	}
	...
	if ( listVersion == AccountListVersion::MojangMSA ) return loadV3(root);
...
bool AccountList::loadV3(QJsonObject& root)
{
	...
	for ( QJsonValue accountVal : accounts )
	{
		...
		if ( account.get() != nullptr )
		{
			...
			if ( profileId.size() )
			{
				if ( findAccountByProfileId(profileId) != -1 ) { continue; }
			}
			connect(account.get(), &MinecraftAccount::changed, this,
			        &AccountList::accountChanged);
			connect(account.get(), &MinecraftAccount::activityChanged, this,
...
bool AccountList::saveList()
{
	...
	{
		qCritical()
		<< "Can't save Mojang account list. No file path given and no default set.";
		return false;
	}

	// make sure the parent folder exists
	if ( !FS::ensureFilePathExists(m_listFilePath) ) return false;
	...
	for ( MinecraftAccountPtr account : m_accounts )
	{
		...
		if ( m_defaultAccount == account ) { accountObj[ "active" ] = true; }
	...
	{
		qCritical() << QString("Failed to save the account list file %1 (%2).")
									 .arg(m_listFilePath)
									 .arg(file.errorString())
									 .toUtf8();
		return false;
...
void AccountList::fillQueue()
{
	...
	{
		auto account = at(i);
		if ( account == m_defaultAccount ) { continue; }
...
void AccountList::requestRefresh(QString accountId)
{
	...
	if ( index != -1 ) { m_refreshQueue.removeAt(index); }
...
{
	if ( m_refreshQueue.indexOf(accountId) != -1 ) { return; }
			...
			if ( account->internalId() == accountId )
			{
				...
				if ( m_currentTask )
				{
					...
					qDebug() << "RefreshSchedule: Processing account"
									 << account->profileName() << "with internal ID" << accountId;
					return;
				}
			}
		}
		qDebug() << "RefreshSchedule: Account with internal ID" << accountId
						 << "not found.";
	}
...
void AccountList::beginActivity()
{
	...
	m_activityCount++;
	if ( activating ) { emit activityChanged(true); }
}
...
void AccountList::endActivity()
{
	...
	m_activityCount--;
	if ( deactivating ) { emit activityChanged(false); }
}
